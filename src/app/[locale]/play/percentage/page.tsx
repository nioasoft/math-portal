...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const title = t('pages.playPercentage.title');
    const description = t('pages.playPercentage.description');

    return {
        title,
        description,
        alternates: generateAlternates('/play/percentage', locale as Locale),
        openGraph: generateOpenGraphMeta(locale as Locale, title, description, '/play/percentage'),
