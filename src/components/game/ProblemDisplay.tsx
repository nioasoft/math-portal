...
export default function ProblemDisplay({ problem }: ProblemDisplayProps) {
    ...
    const FractionDisplay = ({ n, d, whole }: { n: number; d: number; whole?: number }) => (
        <span className="inline-flex items-center gap-1 mx-1">
            {whole && whole > 0 ? <span className="text-4xl font-bold">{whole}</span> : null}
            <span className="inline-flex flex-col items-center text-2xl">
                <span>{n}</span>
                <span className="h-[3px] w-full bg-white my-0.5"></span>
                <span>{d}</span>
            </span>
        </span>
    );
    ...
    if (problem.type === 'fractions' && problem.fractionProblem) {
        ...
        return (
            <div className="text-center py-8">
                <div className="text-4xl md:text-5xl font-bold flex items-center justify-center gap-2" dir="ltr">
                    <FractionDisplay n={f1.n} d={f1.d} whole={f1.whole} />
                    <span className="mx-4 text-yellow-400 text-5xl">{op}</span>
