...
const gameSpecificGuides: Record<string, Record<SeoLocale, string>> = {
  ...
  'decimal-number-line': {
    he: 'הצבת עשרוני על ציר מספרים מפתחת הבנת סדר וגודל — מיומנות יסודית לפני פעולות עם עשרוניים.',
    en: 'Placing a decimal on a number line builds ordering and magnitude sense — a foundation for decimal operations.',
    ar: 'وضع عدد عشري على خط الأعداد يطور فهم الترتيب والحجم — أساس لعمليات الأعداد العشرية.',
    de: 'Eine Dezimalzahl auf dem Zahlenstrahl zu setzen stärkt das Verständnis von Reihenfolge und Größe.',
    es: 'Colocar un decimal en la recta desarrolla el sentido de orden y magnitud — base para operaciones decimales.',
    ru: 'Размещение десятичного числа на прямой развивает понимание порядка и величины.',
  },
  'decimal-addition': {
    he: 'חיבור עשרוניים ספרה אחר ספרה מתרגל העברה בין מקומות — מיומנות קריטית לפני פעולות מורכבות.',
    en: 'Adding decimals digit by digit practices carrying between places — a critical skill before complex operations.',
    ar: 'جمع الأعداد العشرية رقمًا رقمًا يتدرب على التحويل بين الخانات — مهارة حاسمة قبل العمليات المعقدة.',
    de: 'Das stellenweise Addieren von Dezimalzahlen übt den Übergang zwischen Stellen.',
    es: 'Sumar decimales dígito a dígito practica la llevada entre posiciones.',
    ru: 'Последовательное сложение десятичных чисел тренирует переход между разрядами.',
  },
  'number-bond-split': {
  ...
  'percent-of-quantity': {
    he: 'חישוב אחוז מכמות מתרגל הפקת ערך אחוזי מתוך מספר שלם — מיומנות יסודית באחוזים.',
    en: 'Computing a percent of a quantity practices extracting a percentage value from a whole number — a core percent skill.',
    ar: 'حساب نسبة من كمية يتدرب على استخراج قيمة النسبة المئوية من عدد صحيح — مهارة أساسية في النسب.',
    de: 'Das Berechnen eines Prozentsatzes einer Menge übt das Ableiten eines Prozentwerts aus einer ganzen Zahl.',
    es: 'Calcular un porcentaje de una cantidad practica extraer un valor porcentual de un número entero.',
    ru: 'Вычисление процента от числа тренирует извлечение процентного значения из целого числа.',
  },
  'percent-discount': {
    he: 'חישוב מחיר לאחר הנחה מחבר אחוזים לקניות יומיומיות ומתרגל חיסור אחוז מכמות.',
    en: 'Calculating a sale price after discount connects percentages to everyday shopping and practices subtracting a percent from a quantity.',
    ar: 'حساب سعر البيع بعد الخصم يربط النسب المئوية بالتسوق اليومي ويجرب طرح نسبة من كمية.',
    de: 'Das Berechnen eines Verkaufspreises nach Rabatt verbindet Prozente mit dem Alltagseinkauf.',
    es: 'Calcular un precio de venta tras descuento conecta los porcentajes con las compras diarias.',
    ru: 'Вычисление цены после скидки связывает проценты с повседневными покупками.',
  },
  'ratio-mixer': {
  ...
  'ratio-recipe': {
    he: 'הגדלת מתכון לפי כמות מנות מתרגל הכפלת כמויות בפרופורציה — מיומנות בסיסית ביחסים.',
    en: 'Scaling a recipe by servings practices multiplying quantities proportionally — a core ratio skill.',
    ar: 'تغيير الوصفة حسب عدد الحصص يتدرب على ضرب الكميات بالتناسب — مهارة أساسية في النسب.',
    de: 'Das Anpassen eines Rezepts an Portionszahlen übt das proportionale Multiplizieren von Mengen.',
    es: 'Escalar una receta por porciones practica multiplicar cantidades proporcionalmente.',
    ru: 'Масштабирование рецепта по порциям тренирует пропорциональное умножение количеств.',
  },
  'ratio-table': {
    he: 'מילוי טבלת יחסים מתרגל את מושג מקדם ההכפלה — איך אותה כמות נראית בפרופורציות שונות.',
    en: 'Filling a ratio table practices the scaling factor concept — how the same quantity looks at different proportions.',
    ar: 'ملء جدول النسب يتدرب على مفهوم عامل التحجيم — كيف تبدو نفس الكمية بنسب مختلفة.',
    de: 'Das Füllen einer Verhältnistabelle übt den Skalierungsfaktor.',
    es: 'Llenar una tabla de razones practica el concepto de factor de escala.',
    ru: 'Заполнение таблицы отношений тренирует понятие коэффициента масштабирования.',
  },
  'number-sequence': {
  ...
  'geometric-sequence': {
    he: 'סדרה הנדסית מתרגלת זיהוי כלל כפלי — מיומנות שונה מסדרות חיבור ומפתחת חשיבה מתקדמת.',
    en: 'A geometric sequence practices identifying a multiplicative rule — a different skill from additive sequences that builds advanced reasoning.',
    ar: 'المتتالية الهندسية تتدرب على اكتشاف قاعدة الضرب — مهارة مختلفة عن المتتاليات الجمعية.',
    de: 'Eine geometrische Folge übt das Erkennen einer Multiplikationsregel — eine andere Fähigkeit als additive Folgen.',
    es: 'Una secuencia geométrica practica identificar una regla multiplicativa — habilidad distinta a las secuencias aditivas.',
    ru: 'Геометрическая прогрессия тренирует нахождение правила умножения — навык, отличный от аддитивных последовательностей.',
  },
  ...
  'venn-sort': {
    he: 'מיון לפי תכונות חלוקה מפתח חשיבה לוגית וקטגוריזציה — מיומנות יסודית במתמטיקה.',
    en: 'Sorting by divisibility properties develops logical thinking and categorization — a foundational math skill.',
    ar: 'التصنيف حسب خصائص القسمة يطور التفكير المنطقي والتصنيف — مهارة أساسية في الرياضيات.',
    de: 'Sortieren nach Teilbarkeitseigenschaften fördert logisches Denken und Kategorisierung.',
    es: 'Ordenar por propiedades de divisibilidad desarrolla el pensamiento lógico y la categorización.',
    ru: 'Сортировка по свойствам делимости развивает логическое мышление и категоризацию.',
  },
  'subitize-dots': {
  ...
  'word-problem-steps': {
    he: 'בעיות בשני צעדים מפתחות חשיבה רציפה — היכולת לשמור תוצאה ביניים ולהמשיך לחשב.',
    en: 'Two-step problems develop sequential thinking — the ability to hold an intermediate result and continue calculating.',
    ar: 'المسائل بخطوتين تطور التفكير التسلسلي — القدرة على حفظ نتيجة وسيمة والاستمرار في الحساب.',
    de: 'Zweistufige Aufgaben entwickeln sequenzielles Denken — das Festhalten eines Zwischenergebnisses.',
    es: 'Los problemas en dos pasos desarrollan el pensamiento secuencial — mantener un resultado intermedio.',
    ru: 'Задачи в два действия развивают последовательное мышление — удержание промежуточного результата.',
  },
  'estimation-land': {
